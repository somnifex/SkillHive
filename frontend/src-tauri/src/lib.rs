...
pub fn run() {
    tauri::Builder::default()
        .setup(|app| {
            // M1 introduces the first intentional startup side effect: opening
            // and migrating the durable local database. Filesystem deployment,
            // credential access, and network synchronization remain explicit
            // operations and are not started implicitly here.
